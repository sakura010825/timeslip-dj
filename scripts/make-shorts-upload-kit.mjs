...
おすすめ初週の並び（掴みが強く客層が広い順・hide調整可）:
1. #10 ポケベル（31秒・0840=おはようが一瞬で刺さる）
2. #3 プリクラ（背景の質感・女性層）
3. #9 高橋尚子（五輪定番・全世代）
4. #7 バックスクリーン（阪神・関西で強い）
5. #8 生きろ。の夏（ジブリ・フェス世代）
残り（#1六甲道駅 / #2 B'z / #4 ガラケー / #5 コンビニ / #6 対戦台）は2週目に。
`);
...
## #${j.id} ${j.hook}（${j.cell}・${j.window?.dur ? Math.round(j.window.dur) + '秒' : ''}）— 客層: ${j.audience ?? '—'}
